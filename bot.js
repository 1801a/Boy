...
client.on("guildMemberAdd", (member) => {
    let channel = member.guild.channels.get("512398650614349848");
    if (!channel) {
    ...
    var guild;
    while (!guild)
        guild = client.guilds.get("409005637284921374");
    guild.fetchInvites().then((data) => {
        data.forEach((Invite, key, map) => {
            var Inv = Invite.code;
            if (dat[Inv])
...
client.on('guildCreate', guild => {
     ...
     .setURL('https://discordapp.com/api/oauth2/authorize?client_id=512279302855983127&permissions=2146958839&scope=bot')
  .setDescription(`**
...
client.on('message', message => {
  ...
  .setAuthor(message.author.username)
  .setColor("#9B59B6")
  .addField(" ** :gear: Server Support :gear: **" , "  **https://discord.gg/e3DgVHD**")
